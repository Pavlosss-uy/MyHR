...
def run_reinforce(n_simulations: int = 500) -> list:
    ...
    # Use v2 (6-D state) checkpoint; fall back to v1 (3-D) if absent
    engine_6d = AdaptiveDifficultyEngine(state_dim=6)
    engine_3d = AdaptiveDifficultyEngine(state_dim=3)
    engine = engine_6d
    for checkpoint in [
        "models/checkpoints/difficulty_engine_v2.pt",
        ...
        if os.path.exists(checkpoint):
            try:
                ...
                # Dimension mismatch — try 3-D engine
                engine = engine_3d
                try:
                    engine.load_state_dict(torch.load(checkpoint, map_location="cpu", weights_only=True))
                    print(f"Loaded REINFORCE checkpoint (3-D fallback): {checkpoint}")
                    break
                except RuntimeError:
                    continue
    else:
        print("WARNING: no REINFORCE checkpoint found — using random policy")
    ...
    for _ in range(n_simulations):
        ...
        with torch.no_grad():
            for _ in range(env.max_questions):
                result = engine.decide_next_difficulty(
                    episode_scores, len(episode_scores) + 1
                )
                next_diff = result if isinstance(result, int) else result[0]
