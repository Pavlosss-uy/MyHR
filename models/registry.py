...
class ModelRegistry:
    ...
    def load_emotion_model(self):
        ...
        if "emotion" not in self.loaded_models:
            ...
            if self._load_state(model, "emotion"):
                ...
                print("[WARN] Emotion model unavailable — tone analysis will be skipped.")
                self.loaded_models["emotion"] = None
    ...
    def load_evaluator(self):
        ...
        if "evaluator" not in self.loaded_models:
            ...
            if self._load_state(model, "evaluator"):
                print("[OK]   Multi-head evaluator (MOD-4) loaded.")
            else:
                print("[WARN] Evaluator using random-init weights — neural scores will be noisy.")
            model.eval()
